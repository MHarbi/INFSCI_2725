spell_check_dict= matrix(c(
  ...
  'ligth', 'light',
  'sissors', 'scissors'), ncol=2, byrow=T)

spell_check_words <- matrix(, ncol = 2, byrow = T)
spell_check_split <- matrix(, ncol = 2, byrow = T)
for(i in 1:length(spell_check_dict)){
  cat(paste(i, "\n"))
  o <- unlist(strsplit(tolower(spell_check_dict[i,1]), " "))
  c <- unlist(strsplit(tolower(spell_check_dict[i,2]), " "))
  ow <- c()
  cw <- c()
  
  if(length(o) == length(c)){
    for(j in 1:length(o)){
      if(o[j] != c[j]){
        ow <- append(ow, o[j])
        cw <- append(cw, c[j])
    ...
    spell_check_split <- rbind(spell_check_split, c(spell_check_dict[i,1], spell_check_dict[i,2]))
  
  if(length(ow) >= 1 && length(cw) >= 1){
    spell_check_words <- rbind(spell_check_words, c(paste(ow, collapse = " "), paste(cw, collapse = " ")))
  }
}
rm(o,c,ow,cw,i,j)
spell_check_split <- data.frame(spell_check_split[,1], spell_check_split[,2])
write_delim(spell_check_split,"output/spell_check_split.csv", delim ="\t")
