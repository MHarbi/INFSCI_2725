...
grid.newpage()
grid.draw(draw.triple.venn(area1 = length(pidTest), area2 = length(pidTrain), area3 = length(pidAtt), 
														 ...
														 cex = 2,
	                           cat.cex = 2,
														 fill = c("turquoise", "tan1", "skyblue1")))
...
```{r,echo=TRUE}
library(ggplot2)
ggplot(hdTrain,aes(relevance)) + geom_bar(fill="steelblue3") + labs(title="Frequency of Relevance in Train", x="Relevance", y="Frequency")
```
...
hdTrain %>% 
	...
	geom_jitter(alpha=0.2, colour="steelblue3") +
	scale_y_continuous(breaks = round(seq(1,3,by = 0.333),1))+
	labs(title="Average Relevance by Frequency of product_uid in Train")
```
...
hdTrain[c.hasAttribute=="Yes",] %>%
	...
	geom_jitter(alpha=0.05, color="steelblue4") +
	geom_boxplot(color = "steelblue3", outlier.colour = NA, fill = NA)+
	labs(title="Compare relevance by product_uid in attributes file \n and has attribute's name of 'mfg brand name'",x="'mfg brand name' in attributes",y="relevance")
```
...
hdTrain[c.hasAttribute=="Yes",] %>%
	...
	geom_jitter(alpha=0.05, color="royalblue4") +
	geom_boxplot(color = "firebrick1", outlier.colour = NA, fill = NA)+
	labs(title="Compare relevance by product_uid in attributes file \n and has attribute's name of type color",x="Product has color attributes",y="relevance")
```